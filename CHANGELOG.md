...
### Added
- Install-RalphModule.ps1 for one-command module installation (per-user by default, supports -Force and -Scope AllUsers).
- Setup-RalphProject.ps1 now creates RALPH-GETTING-STARTED.md alongside prompt/PRD/progress templates.
- PRD schema generated by setup (plans/prd.schema.json) and documented best practices in README/RALPH.md.
- Default prompt now includes testing/coverage reminders and setup also generates test-coverage-progress.txt.
