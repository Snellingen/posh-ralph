function Invoke-Ralph {
    <#
    .SYNOPSIS
    Runs Ralph (Copilot loop) for multiple iterations.

    .DESCRIPTION
    PowerShell function equivalent to ralph.ps1. Useful when PoshRalph is installed globally.
    #>
    [CmdletBinding()]
    param(
    ...
    function Show-Usage {
        @"
Usage:
  Invoke-Ralph -PromptFile <file> [-PrdFile <file>] [-Skill <a[,b,...]>] [-AllowProfile <safe|dev|locked>] [-AllowTools <toolSpec1>,<toolSpec2>,...] [-DenyTools <toolSpec1>,...] [-Model <model>] -Iterations <N>
...
Usage: Invoke-Ralph -Model <model-name> ...
Example: Invoke-Ralph -Model claude-haiku-4.5 -PromptFile prompts/default.txt -AllowProfile safe -Iterations 10
"@
    }
    ...
    if ([string]::IsNullOrWhiteSpace($PromptFile)) {
        throw "Error: -PromptFile is required"
    }
    ...
    if (-not (Test-Path $PromptFile -PathType Leaf)) {
        throw "Error: prompt file not readable: $PromptFile"
    }

    if (-not [string]::IsNullOrWhiteSpace($PrdFile) -and -not (Test-Path $PrdFile -PathType Leaf)) {
        throw "Error: PRD file not readable: $PrdFile"
    }
